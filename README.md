...
- **حسابات + مزامنة سحابية**: تسجيل دخول (إيميل/باسورد)، خطة المستخدم واشتراكه محفوظين في قاعدة بيانات ومتزامنين عبر كل الأجهزة.
- **قاعدة بيانات**: PostgreSQL على Railway تلقائياً (DATABASE_URL)، أو ملف JSON محلي للتطوير.
